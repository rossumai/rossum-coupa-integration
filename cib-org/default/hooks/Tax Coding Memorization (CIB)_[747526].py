...
class Settings:
    ...
    aliases: Optional[list[dict]] = field(default_factory=list)
    annotation_id: Optional[int] = None
    long_timestamp: Optional[int] = None
    skip_automated_annotations: Optional[bool] = False
    custom_index_name: Optional[str] = "default_all"
    alias_map: dict[str, str] = field(default_factory=dict, init=False, repr=False)
    ...
    def __post_init__(self):
        for f in fields(self):
            ...
            elif "dict[str, str]" in str(f.type):
        ...
        if self.aliases:
            self.alias_map = {item["from_key"]: item["to_key"] for item in self.aliases}
...
class MongoClient:
    ...
    def list_indexes(self, payload):
        return self.rossum.call("POST", f"{self.ds_url}indexes/list", data=json.dumps(payload))

    def create_index(self, payload):
        return self.rossum.call("POST", f"{self.ds_url}indexes/create", data=json.dumps(payload))


def handle_indexes(mongo_client, settings):
    indexes = mongo_client.list_indexes({"collectionName": settings.collection_name, "nameOnly": False})

    if {"$**": 1} not in [i["key"] for i in indexes["result"]]:
        # checking the key and not only the index name, because it can be saved under different name
        new_index = mongo_client.create_index(
            {
                "collectionName": settings.collection_name,
                "indexName": settings.custom_index_name,
                "keys": {"$**": 1},
                "options": {},
            }
        )
        if new_index["code"] == "accept":
            return {"messages": [{"content": "New custom index created", "type": "info"}], "operations": []}
        else:
            return {"messages": [{"content": "Index creation failed", "type": "error"}], "operations": []}
    return {"messages": [{"content": "Custom index was already created, skipping", "type": "info"}], "operations": []}


def rossum_hook_request_handler(payload) -> dict:
    if payload["event"] == "invocation" or (
        ...
        if payload["event"] == "invocation":
            return handle_indexes(mongo_client, settings)
...
def create_documents_from_annotation(t_payload: TxScript, settings: Settings) -> list:
    ...
    if settings.unwind:
        unwind_path = settings.alias_map.get(settings.unwind) or settings.unwind
        simplified_content = unwind(simplified_object=simplified_content[0], paths=[unwind_path])
...
def deduplicate_objects(objects: list) -> list:
    unique_strs = set(json.dumps(obj, sort_keys=True) for obj in objects)
...
def remove_documents(simplified_content: list, settings: Settings) -> list:
    ...
    for doc in simplified_content:
        skip_document = check_document_conditions(
            conditions=settings.skip_record_insert, doc=doc, alias_map=settings.alias_map
        )
...
def check_document_conditions(conditions: list, doc: dict, alias_map: dict) -> Iterator[bool]:
    for condition in conditions:
        yield check_document_condition(condition=condition, doc=doc, alias_map=alias_map)


def check_document_condition(condition: dict, doc: dict, alias_map: dict) -> bool:
    for key in condition:
        if condition[key] != get_value(document=doc, paths=key.split("."), alias_map=alias_map):
            return False
    return True


def get_value(document: dict, paths: list, alias_map: dict) -> Any:
    paths.reverse()
    path = paths.pop()
    aliased_path = alias_map.get(path) or path
    if aliased_path in document and paths:
        value = get_value(document=document[aliased_path], paths=paths, alias_map=alias_map)
    elif aliased_path in document:
        value = document[aliased_path]
    else:
        value = ""
    return value
...
def simplify_content(t_payload: TxScript, settings: Settings) -> list:
    ...
    def get_and_cast_value(
        ...
        value_type = value_type or schema_definition.get("enum_value_type") or schema_definition.get("type")
        if value_type == "date" and not settings.convert_dates:
            ...
            value = round(datapoint, 5) if datapoint.value else None
        else:
    ...
    for key in settings.datapoints_to_save:
        schema_id = key.split(".")[-1]
        final_key = settings.alias_map.get(schema_id) or schema_id
        datapoint = getattr(t_payload.field, schema_id)
        if isinstance(datapoint.attr._field, MultivalueDatapointField):
            ...
            if parent == settings.unwind:
                final_parent_key = settings.alias_map.get(parent) or parent
                for index, datapoint in enumerate(datapoints):
                    value = get_and_cast_value(
                        datapoint=datapoint, schema_definition=datapoint.attr._field.schema, settings=settings
                    )
                    if final_parent_key in simplified_object:
                        if len(simplified_object[final_parent_key]) - 1 >= index:
                            simplified_object[final_parent_key][index][final_key] = value
                        else:
                            simplified_object[final_parent_key].append({final_key: value})
                    else:
                        simplified_object[final_parent_key] = []
                        simplified_object[final_parent_key].append({final_key: value})
        else:
            value = get_and_cast_value(
                datapoint=datapoint, schema_definition=datapoint.attr._field.schema, settings=settings
            )
            simplified_object[final_key] = value
...
def unwind(simplified_object: dict, paths: list) -> list:
    ...
    for path in paths:
        if path not in simplified_object:
            # nothing to unwind, using original object
            transformed_objects.append(template_object)
...
def remove_existing_objects(documents: list, mongo: MongoClient, settings: Settings) -> bool | None:
    ...
    for doc in documents:
        ...
        if settings.unique_natural_key:
            filter_cond = create_filter_query(
                document=doc, natural_key=settings.unique_natural_key, alias_map=settings.alias_map
            )
        if settings.keep_first_record and filter_cond:
            ...
            if len(results["result"]) > 1:
                ...
                for pk in settings.keep_first_record:
                    if get_value(
                        document=oldest_record, paths=pk.split("."), alias_map=settings.alias_map
                    ) != get_value(document=doc, paths=pk.split("."), alias_map=settings.alias_map):
...
def create_filter_query(document: dict, natural_key: list[str], alias_map: dict) -> dict:
    query = {}
    for key in natural_key:
        db_key = ".".join([alias_map.get(i) or i for i in key.split(".")])
        query[db_key] = get_value(document=document, paths=key.split("."), alias_map=alias_map)
    return query
